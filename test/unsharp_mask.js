...
import {glur_mono16, Multimath as mathlib_raw } from '../lib/imports.js'
import mmUnsharpMask from '../lib/mm_unsharp_mask/index.js'
...
describe('unsharp_mask', () => {
  ...
  describe('glur_mono16', () => {
    ...
    it('wasm', () => {
      const glur_js   = glur_mono16
      const mlib_wasm = mathlib_raw({ js: false }).use(mmUnsharpMask);
  ...
  describe('unsharp_mask', () => {
    ...
    it('js should not throw without wasm', () => {
      const mlib = mathlib_raw({ wasm: false }).use(mmUnsharpMask);
    ...
    it('wasm', () => {
      const mlib_js = mathlib_raw({ wasm: false }).use(mmUnsharpMask);
      const mlib_wasm = mathlib_raw({ js: false }).use(mmUnsharpMask);
